   ...
   # text_widget.window_create("20.10", window=label1)
    ...
    if index < len(images):
       ...
       # text_widget.window_create(image_coords[index], window=label1)
        ...
        label1 = tk.Label(text_widget, image=img)
        text_widget.insert(tk.END, "\n\n")
        text_widget.window_create(tk.END, window=label1)
        text_widget.insert(tk.END, "\n\n")

        btn_back = ttk.Button(new_window, text="Tagasi", command=lambda: back_to_previous(new_window, current_window))
        text_widget.window_create(tk.END, window=btn_back)
   # btn_back = ttk.Button(new_window, text="Tagasi", command=lambda: back_to_previous(new_window, current_window))
 #   text_widget.insert("50.0", "\t" * 5)
  #  text_widget.window_create("50.5", window=btn_back)
   ...
   # text_widget.window_create("50.10", window=btn_next)
     ...
     #   text_widget.window_create("50.10", window=btn_next)
        ...
        if index + 1 < len(sections): btn_next = ttk.Button(new_window, text="Edasi", command=lambda: open_window(new_window, index + 1))
        text_widget.insert(tk.END, "\n\n")
        text_widget.window_create(tk.END, window=btn_next)
  #  else:
      #  btn_finish = ttk.Button(new_window, text="Back", command=lambda: back_to_main(new_window))
       # text_widget.insert("50.0", "\t" * 5)
      #  text_widget.window_create("50.10", window=btn_finish)
       # text_widget.insert("50.0", "\t" * 15)
...
def theory_rus(gamesc):
  ...
  #  gamesc3 = tk.Toplevel(gamesc)
   ...
   # with open(resource_path('text.txt'), encoding='utf-8') as file:
    ...
    if sections and images and image_sizes:
        gamesc.withdraw()
        open_window(gamesc, 0)

    #Add a scrolled text widget for displaying the text
   # text_widget = scrolledtext.ScrolledText(gamesc3, wrap=tk.WORD)
    #text_widget.pack(expand=True, fill='both')
    #text_widget.place(x=420, y=50, width=500, height=400)

    #Load text from file
   # global text_sections
    #text_sections = load_text('text.txt')
    #sections = list(text_sections.keys())

    #Show the first section of text
    #show_text(text_widget, sections[0])
...
def theory_est(gamesc2):
   ...
   # gamesc6.geometry("1000x1000")
    ...
    # Create a scrolled text widget for displaying the text
   # text_widget = scrolledtext.ScrolledText(gamesc6, wrap=tk.WORD)
   # text_widget.pack(expand=True, fill='both')


        # Read text from file and insert it into the text widget
   # text = load_text(resource_path('text.txt'))
   ...
   # resize_image = image.resize((350, 250))
    ...
    #text_widget.window_create("20.10", window=label1)
        ...
        # Add the back button and insert it into the text widget
